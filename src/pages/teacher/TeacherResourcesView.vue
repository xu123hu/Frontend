...
<template>
  <div class="resource-v2">
    <header class="resource-v2__head">
      ...
      <button class="t-btn primary lg" type="button" :disabled="store.loading" @click="fileInput?.click()">
        {{ store.loading ? '处理中…' : '上传教学材料' }}
      </button>
      ...
      <section class="resource-v2__catalog" aria-labelledby="catalog-title">
        ...
        <div v-else class="resource-v2__list">
          <article v-for="resource in visibleItems" :key="resource.resource_id" class="resource-v2__item">
            <header>
              <div><p>{{ fileLabel(resource) }} · {{ formatSize(resource.size_bytes) }}</p><h3>{{ resource.name }}</h3></div>
              <span :class="['resource-v2__status', `is-${resource.status}`]">{{ statusText(resource.status) }}</span>
            </header>
            <p class="resource-v2__summary">{{ resource.summary || '尚未生成摘要；原文件仍保持可下载与可追溯。' }}</p>
            <p v-for="warning in resource.warnings || []" :key="warning" class="resource-v2__warning">{{ warning }}</p>
            <div class="resource-v2__actions">
              <button class="t-btn sm" type="button" @click="preprocess(resource.resource_id)">预处理</button>
              <button class="t-btn sm" type="button" @click="understand(resource.resource_id)">生成摘要</button>
              <button class="t-btn sm" type="button" @click="download(resource)">下载原文件</button>
...
async function onPicked(event: Event) { const input = event.target as HTMLInputElement; const file = input.files?.[0]; input.value = ''; if (!file) return; try { await store.upload(file); notice.value = `已上传“${file.name}”。候选题仍需教师审核。`; showToast('上传成功') } catch (e: any) { showToast(e?.message || '上传失败') } }
...
.resource-v2__head, .resource-v2__section-head, .resource-v2__item > header { display: flex; align-items: flex-start; justify-content: space-between; gap: 20px; }
