<template>
  <aside class="rubric-panel" data-grading-region="rubric" aria-label="评分量规与教师决定">
    <section class="rubric-panel__summary">
      ...
      <LatexText v-if="selection.scoring.standardAnswer" class="rubric-panel__standard" :text="selection.scoring.standardAnswer" />
      <LatexText v-if="selection.scoring.answerAnalysis" class="rubric-panel__analysis" :text="selection.scoring.answerAnalysis" />
    </section>
    ...
    <section class="rubric-panel__rubric">
      ...
      <ol v-else>
        <li v-for="item in selection.scoring.rubricItems" :key="item.id">
          <span><strong><LatexText :text="item.criterion" /></strong><small><LatexText :text="item.evidenceHint" /></small></span>
          <b>{{ item.points }} 分</b>
        </li>
      </ol>
    </section>
    ...
    <section class="rubric-panel__evidence">
      ...
      <p v-for="evidence in selection.suggestion?.evidence || []" :key="evidence.kind + evidence.text"><LatexText :text="evidence.text" /></p>
    ...
    <section class="rubric-panel__decision">
      ...
      <input id="grading-final-score" :value="finalScore ?? ''" type="number" min="0" :max="selection.scoring.maxScore ?? undefined" step="0.5" @input="updateScore" />
...
.rubric-panel__standard { display:block; margin:13px 0 7px; color:#233b58; font-size:13px; line-height:1.6; }
.rubric-panel__analysis { display:block; margin:0; color:#66778c; font-size:12px; line-height:1.55; }
