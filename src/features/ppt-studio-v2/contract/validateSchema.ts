/**
 * 轻量 JSON Schema 校验器（R10 前端契约测试用）。
 *
 * 只覆盖 TeachingDeckSpec 生成 schema 实际用到的子集（见 deck-spec.schema.json）：
 *   type(含数组型) / enum / const / $ref($defs) / anyOf(含 null) /
 *   array+items+minItems / object(properties+required+additionalProperties) /
 *   Record<string,T>(additionalProperties 无 properties) / 数值 minimum。
      ...
      if (node.additionalProperties === false) {
        for (const key of Object.keys(record)) {
          if (!(key in node.properties)) {
            errors.push(atPath(path, `未知字段 ${key}`))
          }
        }
      }
