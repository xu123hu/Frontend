...
`PW_PORT=5192 npx playwright test e2e/teacher-grading-v2.spec.ts --project=chromium --workers=1` completed with **3 passed**:
...
The original-work surface renders inline TeX through the repository's existing sanitized `LatexText` component, rather than presenting source such as `\\pm` or `\\infty` in a plain-text block. The focused component test asserts the visible KaTeX output contains `x=±1` and not the raw `\\pm` sequence; the fresh 1366×768 browser screenshot shows the same outcome. Queue labels are supplied by the server as `匿名作答 #001` so the number is clearly an anonymous blind-grading identifier, not a student name or an unexplained assignment number.
